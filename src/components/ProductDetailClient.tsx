...
function formatPublishedAt(dt?: string | Date | null, locale: string = 'en-US'): string {
   if (!dt) return ''
   try {
     const d = new Date(dt)
  ...
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
      ...
      <div>
        ...
        {categoryName && (
          <p className="mt-2 text-gray-600">{t(lang, 'product.category')}: {categoryName}</p>
        )}
        {brand && (
          <p className="mt-1 text-gray-600">{t(lang, 'product.brand')}: {brand}</p>
        )}
        {upc && (
          <p className="mt-1 text-gray-600">{t(lang, 'product.upc')}: {upc}</p>
        )}
        {asin && (
          <p className="mt-1 text-gray-600">{t(lang, 'product.asin')}: {asin}</p>
        )}
        {publishedAt && (
          <p className="mt-1 text-gray-500 text-sm">{t(lang, 'product.dateFirstAvailable')} : {formatPublishedAt(publishedAt, locale)}</p>
        )}

        {/* 变体选项挪到价格上方，优先图片显示 */}
        {Array.isArray(variantGroups) && variantGroups.length > 0 && (
          <div className="mt-6">
            <h3 className="text-md font-semibold text-gray-900">{t(lang, 'product.availableOptions')}</h3>
            <div className="mt-3 space-y-4">
              {variantGroups.map((group, gi) => (
                <div key={gi}>
                  <p className="text-sm text-gray-700">{translateAttributeName(group.name, lang)}</p>
    ...
    <div className="mt-10">
      <h2 className="text-lg font-semibold text-gray-900">{t(lang, 'product.description')}</h2>
      <div 
    ...
    {Array.isArray(sortedReviews) && sortedReviews.length > 0 && (
      <div className="mt-10">
        <h2 className="text-lg font-semibold text-gray-900">{t(lang, 'product.customerReviews')}</h2>
        <div className="mt-2 flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input type="checkbox" checked={onlyWithImages} onChange={(e) => setOnlyWithImages(e.target.checked)} />
            {t(lang, 'product.onlyReviewsWithImages')}
          </label>
          <div className="flex items-center gap-2 text-sm text-gray-700">
            <span>{t(lang, 'product.sort')}</span>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as 'time' | 'rating')} className="border rounded px-2 py-1">
              <option value="time">{t(lang, 'product.newest')}</option>
              <option value="rating">{t(lang, 'product.highestRating')}</option>
            </select>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
          <span>
            {sortedReviews.length > 0
              ? t(lang, 'product.showingRange', { start: (currentReviewPage - 1) * REVIEWS_PER_PAGE + 1, end: Math.min(currentReviewPage * REVIEWS_PER_PAGE, sortedReviews.length), total: sortedReviews.length })
              : t(lang, 'product.showingZero')}
          </span>
          <div className="flex items-center gap-2">
            <button
              type="button"
    ...
    {previewUrls && (
      <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center" onClick={() => setPreviewUrls(null)}>
        <button type="button" className="absolute top-3 right-3 bg-black/60 text-white rounded px-3 py-1 text-sm" onClick={(e) => { e.stopPropagation(); setPreviewUrls(null) }}>{t(lang, 'product.close')}</button>

...
import { normalizeFrontendLanguage, t, translateNavLabel } from '@/lib/i18n'
              ...
              itemsToRender.map((item) => (
                item.href === '/products' ? (
                  <div key={item.id} className="relative group">
                    <NavLink href={item.href}>{translateNavLabel(item.label, lang)}</NavLink>
                    {categories.length > 0 && (
                      <div className="absolute left-0 top-full pt-2 w-48 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all z-50">
                        <div className="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                          <Link href="/products" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">{t(lang, 'nav.allProducts')}</Link>
                          {categories.map((c) => (
                  ...
                  <NavLink key={item.id} href={item.href}>
                    {translateNavLabel(item.label, lang)}
                  </NavLink>
                )
              ))
            )}
          ...
          <div className="flex md:flex-1 justify-end items-center gap-2">
            ...
            <form action="/products" method="get" className="hidden md:flex items-center gap-2 bg-gray-100 rounded-lg px-2 py-1">
              <input
                type="text"
                name="q"
                placeholder={t(lang, 'nav.searchProducts')}
                className="bg-transparent px-2 py-1 text-sm focus:outline-none w-40 md:w-48"
              />
              <button type="submit" className="text-sm px-2 py-1 rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300">{t(lang, 'nav.search')}</button>
            </form>
            ...
            >
              ...
              <span className="ml-2">{t(lang, 'nav.cart')}</span>
            </Link>
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1 bg-white/95 backdrop-blur-md border-t border-gray-200/50">
            ...
            <form action="/products" method="get" className="flex items-center gap-2 bg-gray-100 rounded-lg px-2 py-2 mb-2">
              <input type="text" name="q" placeholder={t(lang, 'nav.searchProducts')} className="flex-1 bg-transparent px-2 py-1 text-sm focus:outline-none" />
              <button type="submit" className="text-sm px-2 py-1 rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300">{t(lang, 'nav.search')}</button>
            </form>
            {showSkeleton ? (
              ...
              <>
                ...
                {categories.length > 0 && (
                  <div className="mt-2 bg-white rounded-xl border border-gray-200">
                    <div className="px-4 py-2 text-xs text-gray-500">{t(lang, 'nav.browseByCategory')}</div>
                    {categories.map((c) => (
